...
public class Ex05DynamicAlloc {
   ...
   static boolean contains(int[] arr, int element) {
	   ...
	   for(int i = 0; i < arr.length; i ++) {
		   if(arr[i] == element) {
			   //1번째 인덱스와 element의 값이 같으므로
			   //return true 해준다.
			   return true;
			   // 메소드는 return 을 만나는 순간 메소드가 종료되어 return 값을 반환함
		   }
	   }
	   return false;
   }
   ...
   static int indexOf(int[] arr, int element) {
	   //여기서 여러분들이 한가지 고민해 볼 것은
	   //만약 element 가 arr에 존재하지 않는다면
	   //우리가 어떠한 값을 리턴해주어야 할까?
	   //인덱스는 무조건 0 부터 arr의 크기 -1 이기 때문에
	   //양의 정수로 우리가 존재하지 않는다는 표시를 해주기 힘들다.
   ...
   static int[] removeByIndex(int[] arr, int index) {
	   ...
	   // arr을 새로 초기화해주자
	   // 이때 arr의 크기는 얼마인가?
	   arr = new int[arr.length - 1]; //arr 의 인덱스의 크기 -1 을 한 값.
	   // 왜냐하면 인덱스에 해당하는 값 하나를 제거하면 크기가 하나 줄어야 하기 때문
	   
	   //만약 해당 인덱스가 존재하면
	   //우리는 3가지 경우에 놓인다.
	   //1. 인덱스가 0일 때
	   //	인덱스가 0이면 우리는 새로 만든 arr에 temp번부터 끝까지 복사해주면 된다!
	   ...
	   if(index == 0) {
		   ...
		   //2. 인덱스가 가장 마지막일 때
		   //이때는 temp의 0번부터 temp.length - 2 까지를
		   //arr에 복사해주면 된다.
		   ...
		   for(int i = 0; i < temp.length; i ++) {
			   if(i < index) {
				   ...
				   // arr[7] 또한 위에서 사이즈를 1줄여 놓았으므로 크기가 7 이기 때문에 두 값 모두 에러가 발생되는 인덱스 값을 가지게 되는것임.
   ...
   //파라미터로 넘어온element를 배열에서 삭제해서 리턴해주는 removeByElement()
